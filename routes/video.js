...
router.post('/getVideoDescription', async (req, res) => {
	if (!req.user) {
		...
		try {
			...
			console.log('iDisliked ' + iDisliked);
...
router.post('/upload', async (req, res) => {
	...
	try {
		...
		busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {			// file is an event listener , is fired
			...
			} else if (mimetype == 'text/plain' || mimetype == 'text/srt' || mimetype == 'application/x-subrip') {
				subtitleOk = 1;
				fstream = fs.createWriteStream(path.join(uploadPath, subtitleName + '.srt'));
				console.log("My mime tyep is " + mimetype);
			} else {
				file.resume();
				return;
			}
		...
		busboy.on('finish',async function () {
			...
			console.log('subtitleOk ' + subtitleOk);
			if(thumbnailOk != 1){
			...
			if (thumbnailOk != 1 || videoOk != 1) {
				...
				if (subtitleOk == 1) {
					fs.unlink('./assets/' + subtitleName + '.srt');
				}
